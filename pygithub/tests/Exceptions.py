...
class Exceptions(Framework.TestCase):
    ...
    def testBadAuthentication(self):
        with self.assertRaises(pygithub.GithubException) as raisedexp:
            pygithub.Github(auth=pygithub.Auth.Login("BadUser", "BadPassword")).get_user().login
    ...
    def testExceptionPickling(self):
        pickle.loads(pickle.dumps(pygithub.GithubException("foo", "bar", None)))
...
class SpecificExceptions(Framework.TestCase):
    def testBadCredentials(self):
        self.assertRaises(
            ...
            lambda: pygithub.Github(auth=pygithub.Auth.Login("BadUser", "BadPassword")).get_user().login,
        )

    def test2FARequired(self):
        self.assertRaises(
            pygithub.TwoFactorException,
            lambda: pygithub.Github(auth=pygithub.Auth.Login("2fauser", "password")).get_user().login,
        )
    ...
    def testBadUserAgent(self):
        self.assertRaises(
            ...
            lambda: pygithub.Github(auth=self.login, user_agent="").get_user().name,
        )

    def testRateLimitExceeded(self):
        # rate limit errors would be retried if retry is not set None
        g = pygithub.Github(retry=None)
    ...
    def testAuthenticatedRateLimitExceeded(self):
        ...
        with self.assertRaises(pygithub.RateLimitExceededException) as raised:
            exceed()
        self.assertEqual(raised.exception.headers.get("retry-after"), "60")

    def testIncompletableObject(self):
        pygithub.UserKey.UserKey.setCheckAfterInitFlag(False)
        obj = pygithub.UserKey.UserKey(None, {}, {}, False)
        self.assertRaises(pygithub.IncompletableObject, obj._completeIfNeeded)
