...
class Requester(Framework.TestCase):
    ...
    def setUp(self):
        ...
        pygithub.Requester.Requester.injectLogger(self.logger)

    def tearDown(self):
        pygithub.Requester.Requester.resetLogger()
        super().tearDown()

    def testRecreation(self):
        class TestAuth(pygithub.Auth.AppAuth):
            pass

        # create a Requester with non-default arguments
        auth = TestAuth(123, "key")
        requester = pygithub.Requester.Requester(
            auth=auth,
        ...
        self.assertEqual(kwargs.keys(), pygithub.Requester.Requester.__init__.__annotations__.keys())
        self.assertEqual(
            kwargs,
            dict(
                auth=auth,
    ...
    def testCloseGithubIntegration(self):
        ...
        with pygithub.GithubIntegration(auth=auth) as gi:
    ...
    def testBaseUrlSchemeRedirection(self):
        gh = pygithub.Github(base_url="http://api.github.com")
    ...
    def testBaseUrlHostRedirection(self):
        gh = pygithub.Github(base_url="https://www.github.com")
        with self.assertRaises(RuntimeError) as exc:
            gh.get_repo("PyGithub/PyGithub")
        self.assertEqual(
            exc.exception.args,
            (
                "Github server redirected from host www.pygithub.com to pygithub.com, "
    ...
    def testBaseUrlPortRedirection(self):
        # replay data forged
        gh = pygithub.Github(base_url="https://api.github.com")
        with self.assertRaises(RuntimeError) as exc:
            gh.get_repo("PyGithub/PyGithub")
        self.assertEqual(
            exc.exception.args,
    ...
    def testBaseUrlPrefixRedirection(self):
        # replay data forged
        gh = pygithub.Github(base_url="https://api.github.com/api/v3")
    ...
    def testIsRateLimitError(self):
        for message in self.PrimaryRateLimitErrors + self.SecondaryRateLimitErrors:
            self.assertTrue(pygithub.Requester.Requester.isRateLimitError(message), message)
        for message in self.OtherErrors:
            self.assertFalse(pygithub.Requester.Requester.isRateLimitError(message), message)

    def testIsPrimaryRateLimitError(self):
        for message in self.PrimaryRateLimitErrors:
            self.assertTrue(pygithub.Requester.Requester.isPrimaryRateLimitError(message), message)
        for message in self.OtherErrors + self.SecondaryRateLimitErrors:
            self.assertFalse(pygithub.Requester.Requester.isPrimaryRateLimitError(message), message)

    def testIsSecondaryRateLimitError(self):
        for message in self.SecondaryRateLimitErrors:
            self.assertTrue(pygithub.Requester.Requester.isSecondaryRateLimitError(message), message)
        for message in self.OtherErrors + self.PrimaryRateLimitErrors:
            self.assertFalse(pygithub.Requester.Requester.isSecondaryRateLimitError(message), message)
    ...
    def testShouldCreateBadCredentialsException(self):
        ...
        self.assertException(
            exc,
            pygithub.BadCredentialsException,
            401,
    ...
    def testShouldCreateBadUserAgentException(self):
        ...
        self.assertException(
            exc,
            pygithub.BadUserAgentException,
            403,
            {"message": "Missing or invalid User Agent string"},
            {"header": "value"},
    ...
    def testShouldCreateUnknownObjectException(self):
        ...
        self.assertException(
            exc,
            pygithub.UnknownObjectException,
            404,
            {"message": "Not Found"},
            {"header": "value"},
    ...
    def testShouldCreateExceptionWithoutMessage(self):
        for status in range(400, 600):
            with self.subTest(status=status):
                ...
                self.assertException(exc, pygithub.GithubException, status, {}, {}, f"{status} {{}}")

    def testShouldCreateExceptionWithoutOutput(self):
        for status in range(400, 600):
            with self.subTest(status=status):
                exc = self.g._Github__requester.createException(status, {}, None)
                self.assertException(exc, pygithub.GithubException, status, None, {}, f"{status}")
...
class RequesterThrottleTestCase(Framework.TestCase):
    ...
    def mock_sleep(self):
        with mock.patch("pygithub.Requester.time.sleep", side_effect=self.sleep) as sleep_mock, mock.patch(
            "pygithub.Requester.datetime"
        ) as datetime_mock:
    ...
    def testShouldDeferWrites(self):
        with self.mock_sleep() as sleep_mock:
            ...
            self.assertEqual(
                ...
                ["vincent@vincent-jacques.net", "pygithub.com@vincent-jacques.net"],
            )
            ...
            self.assertEqual(
                ...
                [
                    ...
                    "1@foobar.com",
                    "2@foobar.com",
                    "pygithub.com@vincent-jacques.net",
                ],
            )
