...
from pygithub.Auth import AppInstallationAuth
...
class GithubIntegration(Framework.BasicTestCase):
    ...
    def testDeprecatedAppAuth(self):
        ...
        with self.assertWarns(DeprecationWarning) as warning:
            github_integration = pygithub.GithubIntegration(integration_id=APP_ID, private_key=PRIVATE_KEY)
        ...
        self.assertWarning(
            ...
            "jwt_algorithm are deprecated, please use auth=pygithub.Auth.AppAuth(...) "
            "instead",
        )
    ...
    def testRequiredAppAuth(self):
        ...
        for auth in [self.oauth_token, self.jwt, self.login]:
            with self.assertRaises(AssertionError) as r:
                pygithub.GithubIntegration(auth=auth)
            self.assertEqual(
                str(r.exception),
    ...
    def testAppAuth(self):
        ...
        github_integration = pygithub.GithubIntegration(auth=auth)
        installations = github_integration.get_installations()
        self.assertEqual(len(list(installations)), 2)

    def testNoneAppAuth(self):
        with self.assertRaises(AssertionError):
            pygithub.GithubIntegration(auth=None)
    ...
    def testGetGithubForInstallation(self):
        ...
        with self.ignoreWarning(category=InsecureRequestWarning, module="urllib3.connectionpool"):
            ...
            github_integration = pygithub.GithubIntegration(**kwargs)
    ...
    def testGetInstallationNotFound(self):
        auth = pygithub.Auth.AppAuth(APP_ID, PRIVATE_KEY)
        github_integration = pygithub.GithubIntegration(auth=auth)
        with self.assertRaises(pygithub.UnknownObjectException) as raisedexp:
