...
class Enterprise(Framework.BasicTestCase):
    def testHttps(self):
        g = pygithub.Github(auth=self.login, base_url="https://my.enterprise.com")
    ...
    def testHttp(self):
        g = pygithub.Github(auth=self.login, base_url="http://my.enterprise.com")
        self.assertListKeyEqual(
            g.get_user().get_repos(),
            lambda r: r.name,
            [
    ...
    def testUnknownUrlScheme(self):
        with self.assertRaises(AssertionError) as raisedexp:
            pygithub.Github(auth=self.login, base_url="foobar://my.enterprise.com")
        self.assertEqual(raisedexp.exception.args[0], "Unknown URL scheme")

    def testLongUrl(self):
        g = pygithub.Github(auth=self.login, base_url="http://my.enterprise.com/path/to/github")
    ...
    def testSpecificPort(self):
        g = pygithub.Github(auth=self.login, base_url="http://my.enterprise.com:8080")
        self.assertListKeyEqual(
            g.get_user().get_repos(),
            lambda r: r.name,
            [
