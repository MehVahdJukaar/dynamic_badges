...
class RecordingHttpConnection(RecordingConnection):
    _realConnection = pygithub.Requester.HTTPRequestsConnectionClass
...
class RecordingHttpsConnection(RecordingConnection):
    _realConnection = pygithub.Requester.HTTPSRequestsConnectionClass
...
class BasicTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        if (
            ...
            self.login = (
                pygithub.Auth.Login(GithubCredentials.login, GithubCredentials.password)
                if GithubCredentials.login and GithubCredentials.password
                else None
            )
            self.oauth_token = (
                pygithub.Auth.Token(GithubCredentials.oauth_token) if GithubCredentials.oauth_token else None
            )
            self.jwt = pygithub.Auth.AppAuthToken(GithubCredentials.jwt) if GithubCredentials.jwt else None
            self.app_auth = (
                pygithub.Auth.AppAuth(GithubCredentials.app_id, GithubCredentials.app_private_key)
            ...
            self.login = pygithub.Auth.Login("login", "password")
            self.oauth_token = pygithub.Auth.Token("oauth_token")
            self.jwt = pygithub.Auth.AppAuthToken("jwt")
            self.app_auth = pygithub.Auth.AppAuth(123456, APP_PRIVATE_KEY)
