...
# Copyright 2016 Martijn Koster <mak-pygithub@greenhills.co.uk>                  #
...
# Copyright 2020 chloe jungah kim <43295963+chloeeekim@users.noreply.pygithub.com>#
...
class ProjectCard(CompletableGithubObject):
    ...
    def _initAttributes(self) -> None:
        ...
        self._creator: Attribute[pygithub.NamedUser.NamedUser] = NotSet
    ...
    @property
    def creator(self) -> pygithub.NamedUser.NamedUser:
    ...
    def get_content(
    ...
    ) -> pygithub.PullRequest.PullRequest | pygithub.Issue.Issue | None:
        """
        ...
        retclass: type[pygithub.PullRequest.PullRequest] | type[pygithub.Issue.Issue]
        if content_type == "PullRequest":
            url = self.content_url.replace("issues", "pulls")
            retclass = pygithub.PullRequest.PullRequest
        elif content_type is NotSet or content_type == "Issue":
            url = self.content_url
            retclass = pygithub.Issue.Issue
        else:
    ...
    def move(self, position: str, column: pygithub.ProjectColumn.ProjectColumn | int) -> bool:
        """
        ...
        assert isinstance(column, pygithub.ProjectColumn.ProjectColumn) or isinstance(column, int), column
        post_parameters = {
            "position": position,
            "column_id": column.id if isinstance(column, pygithub.ProjectColumn.ProjectColumn) else column,
        }
