...
class SimpleCredit(TypedDict):
    ...
    login: str | pygithub.NamedUser.NamedUser
    type: str
...
class AdvisoryCredit(NonCompletableGithubObject):
    ...
    def _validate_credit(credit: Credit) -> None:
        ...
        if isinstance(credit, dict):
            ...
            assert isinstance(credit["login"], (str, pygithub.NamedUser.NamedUser)), credit["login"]
    ...
    def _to_github_dict(credit: Credit) -> SimpleCredit:
        ...
        if isinstance(credit, dict):
            ...
            assert isinstance(credit["login"], (str, pygithub.NamedUser.NamedUser)), credit["login"]
            login = credit["login"]
            if isinstance(login, pygithub.NamedUser.NamedUser):
                login = login.login
            return {
                "login": login,
                "type": credit["type"],
