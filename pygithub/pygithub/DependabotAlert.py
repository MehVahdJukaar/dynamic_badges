...
import pygithub.AdvisoryVulnerabilityPackage
import pygithub.DependabotAlertAdvisory
import pygithub.DependabotAlertDependency
import pygithub.DependabotAlertVulnerability
import pygithub.NamedUser
from pygithub.GithubObject import Attribute, NonCompletableGithubObject, NotSet

if TYPE_CHECKING:
    from pygithub.DependabotAlertAdvisory import DependabotAlertAdvisory
    from pygithub.DependabotAlertDependency import DependabotAlertDependency
    from pygithub.DependabotAlertVulnerability import DependabotAlertVulnerability
    ...
    def _useAttributes(self, attributes: dict[str, Any]) -> None:
        ...
        if "dependency" in attributes:
            self._dependency = self._makeClassAttribute(
                pygithub.DependabotAlertDependency.DependabotAlertDependency, attributes["dependency"]
            )
        if "security_advisory" in attributes:
            self._security_advisory = self._makeClassAttribute(
                pygithub.DependabotAlertAdvisory.DependabotAlertAdvisory, attributes["security_advisory"]
            )
        if "security_vulnerability" in attributes:
            self._security_vulnerability = self._makeClassAttribute(
                pygithub.DependabotAlertVulnerability.DependabotAlertVulnerability, attributes["security_vulnerability"]
            )
