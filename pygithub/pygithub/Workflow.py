...
class Workflow(CompletableGithubObject):
    ...
    def create_dispatch(
        self, ref: pygithub.Branch.Branch | pygithub.Tag.Tag | pygithub.Commit.Commit | str, inputs: Opt[dict] = NotSet
    ) -> bool:
        """
        ...
        """
        assert (
            isinstance(ref, pygithub.Branch.Branch)
            or isinstance(ref, pygithub.Tag.Tag)
            or isinstance(ref, pygithub.Commit.Commit)
            or isinstance(ref, str)
        ), ref
        assert inputs is NotSet or isinstance(inputs, dict), inputs
        if isinstance(ref, pygithub.Branch.Branch):
            ref = ref.name
        elif isinstance(ref, pygithub.Commit.Commit):
            ref = ref.sha
        elif isinstance(ref, pygithub.Tag.Tag):
            ref = ref.name
        if inputs is NotSet:
            inputs = {}
    ...
    def get_runs(
        self,
        actor: Opt[pygithub.NamedUser.NamedUser | str] = NotSet,
        branch: Opt[pygithub.Branch.Branch | str] = NotSet,
    ...
    ) -> PaginatedList[pygithub.WorkflowRun.WorkflowRun]:
        """
        ...
        """
        assert actor is NotSet or isinstance(actor, pygithub.NamedUser.NamedUser) or isinstance(actor, str), actor
        assert branch is NotSet or isinstance(branch, pygithub.Branch.Branch) or isinstance(branch, str), branch
        ...
        if actor is not NotSet:
            url_parameters["actor"] = actor._identity if isinstance(actor, pygithub.NamedUser.NamedUser) else actor
        if branch is not NotSet:
            url_parameters["branch"] = branch.name if isinstance(branch, pygithub.Branch.Branch) else branch
