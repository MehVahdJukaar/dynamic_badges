...
import pygithub.AccessToken
import pygithub.Auth
from pygithub.GithubException import BadCredentialsException, GithubException
from pygithub.GithubObject import Attribute, NonCompletableGithubObject, NotSet
from pygithub.Requester import Requester

if TYPE_CHECKING:
    from pygithub.AccessToken import AccessToken
    from pygithub.Auth import AppUserAuth
...
class ApplicationOAuth(NonCompletableGithubObject):
    ...
    def get_access_token(self, code: str, state: str | None = None) -> AccessToken:
        ...
        return pygithub.AccessToken.AccessToken(
    ...
    def get_app_user_auth(self, token: AccessToken) -> AppUserAuth:
        return pygithub.Auth.AppUserAuth(
            client_id=self.client_id,
            client_secret=self.client_secret,
            token=token.token,
