...
# Copyright 2016 Matt Babineau <babineaum@users.noreply.pygithub.com>            #
...
import pygithub.Reaction
import pygithub.Repository
import pygithub.TimelineEvent
...
if TYPE_CHECKING:
    ...
    from pygithub.IssuePullRequest import IssuePullRequest
    ...
    from pygithub.Reaction import Reaction
    from pygithub.Repository import Repository
    from pygithub.TimelineEvent import TimelineEvent
...
class Issue(CompletableGithubObject):
    ...
    def repository(self) -> Repository:
        ...
        if is_undefined(self._repository):
            ...
            self._repository = pygithub.GithubObject._ValuedAttribute(
                pygithub.Repository.Repository(self._requester, self._headers, {"url": repo_url}, completed=False)
            )
    ...
    def edit(
        ...
        assert assignee is None or is_optional(assignee, (pygithub.NamedUser.NamedUser, str)), assignee
        assert is_optional_list(assignees, (pygithub.NamedUser.NamedUser, str)), assignees
        assert is_optional(state, str), state
        assert milestone is None or is_optional(milestone, pygithub.Milestone.Milestone), milestone
    ...
    def create_reaction(self, reaction_type: str) -> Reaction:
        ...
        return pygithub.Reaction.Reaction(self._requester, headers, data, completed=True)
    ...
    def get_timeline(self) -> PaginatedList[TimelineEvent]:
        ...
        """
        return PaginatedList(
            pygithub.TimelineEvent.TimelineEvent,
            self._requester,
            f"{self.url}/timeline",
            None,
    ...
    def _useAttributes(self, attributes: dict[str, Any]) -> None:
        ...
        if "pull_request" in attributes:  # pragma no branch
            self._pull_request = self._makeClassAttribute(
                pygithub.IssuePullRequest.IssuePullRequest, attributes["pull_request"]
            )
